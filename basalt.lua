...
return
{basalt=function(ac)
local function bc(dc,_d)local ad=_d[dc.tag]
...
cd[dd](cd,function(...)
local a_a=ac.getVariable(__a:gsub("\"",""):gsub("\'",""))if(a_a~=nil)then a_a()elseif(_d[__a]~=nil)then _d[__a]()else
_c(cd,cd[dd],__a.."()",_d)end end)else
local a_a=function()local b_a=load("return "..__a,nil,"t",_d)()
cd:setProperty(dd,b_a)end;ac.effect(a_a)end end;for dd,__a in ipairs(dc.children)do local a_a=bc(__a,_d)
for b_a,c_a in ipairs(a_a)do cd:addChild(c_a)end end;return{cd}end
...
setmetatable(ad.props,{__index=function(dd,__a)
if(bd[__a]==nil)then error("Property "..__a..
" not found")end;return bd[__a]()end})if(dc.script~=nil)then db(dc.script,ad)end;local cd={}for dd,__a in
...
local bca=function()return cb end;local cca=function(adb)return bca()[adb]end;local dca=function(adb,bdb)return
cca(adb)(bdb,aba)end
local _da={getDynamicValueEventSetting=function()
return aba.dynamicValueEvents end,getMainFrame=function()return baa end,setVariable=_ca,getVariable=aca,setMainFrame=function(adb)baa=adb end,getActiveFrame=function()return caa end,setActiveFrame=function(adb)
caa=adb end,getFocusedObject=function()return daa end,setFocusedObject=function(adb)daa=adb end,getMonitorFrame=function(adb)return d_a[adb]or
monGroups[adb][1]end,setMonitorFrame=function(adb,bdb,cdb)
if(baa==bdb)then baa=nil end
if(cdb)then monGroups[adb]={bdb,sides}else d_a[adb]=bdb end;if(bdb==nil)then monGroups[adb]=nil end end,getTerm=function()return
...
local cdb,ddb=xpcall(function()dbb(adb,table.unpack(bdb))end,debug.traceback)if not(cdb)then aba.basaltError(ddb)return end end end,stop=cba,stopUpdate=cba,getTerm=function()return
dd end,isKeyDown=function(adb)if(b_a[adb]==nil)then return false end;return b_a[adb]end,getFrame=function(adb)for bdb,cdb in
...
aa["objects"]["Button"]=function(...)local ab=da("utils")
local bb=da("tHex")
return
function(cb,db)local _c=db.getObject("VisualObject")(cb,db)
local ac="Button"local bc="center"local cc="center"local dc="Button"_c:setSize(12,3)
_c:setZIndex(5)
...
pairs(table.pack(...))do
if(type(c_a)=="function")then a_a:registerEvent("key_up",c_a)end end;return a_a end}__a.__index=__a;return __a end end;return aa["main"]()